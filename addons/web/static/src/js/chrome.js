...
instance.web.CrashManager = instance.web.Class.extend({
    ...
    rpc_error: function(error) {
        ...
        if (error.data.name === "openerp.http.SessionExpiredException" || error.data.name === "werkzeug.exceptions.Forbidden") {
            this.show_warning({type: "Session Expired", data: { message: _t("Phiên làm việc trên CTTMS đã hết. Vui lòng bấm F5 để tạo phiên mới.") }});
            return;
        }
    ...
    show_warning: function(error) {
        ...
        new instance.web.Dialog(this, {
            size: 'medium',
            title: "CTTMS " + (_.str.capitalize(error.type) || " Cảnh báo"),
            buttons: [
...
instance.web.UserMenu =  instance.web.Widget.extend({
    ...
    on_menu_help: function() {
        window.open('http://help.cttms.vn', '_blank');
    },
    ...
    on_menu_account: function() {
        ...
        if (!this.getParent().has_uncommitted_changes()) {
            ...
            P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                ...
                instance.web.redirect('https://accounts.cttms.vn/oauth2/auth?'+$.param(params));
            }).fail(function(result, ev){
                ev.preventDefault();
                instance.web.redirect('https://accounts.cttms.vn/web');
            });
        }
    },
